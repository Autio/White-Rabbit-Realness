...
public class GameController : Singleton<GameController>
{
    ...
    void NextQuestion(int activePhase, int[] questionsAsked)
    {
        // The game has been passed
        if(questionsAsked.Length > questionLimit)
        {
            WinGame();
        }
        ...
        {
            GameObject newAnswerOption = Instantiate(answerOption, answerOptionAnchors[i], Quaternion.Identity) as GameObject;
            ActiveOptions.Add(newAnswerOption);
            // Set text 
            newAnswerOption.GetComponent<Text>().text = question.answers[i];            
        }
    }
